    ...
    ?>
      <tr>
        ...
        <td>
          ...
          if ($diff == "1") {
          ?>
            <i class="fa fa-exclamation-triangle" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Grace Period" style="color:#deec00;text-decoration:none;"></i>&nbsp;
          <?php } else if (date('Y-m-d', time()) > date('Y-m-d', strtotime($cl->end_date))) {
            echo '<i class="fa fa-exclamation-triangle" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Expired" style="color: red;"></i>&nbsp;';
          } ?>
          <?=$cl->client_name;?>
        </td>
        ...
        <td>
          <a target="_blank" href="<?=base_url('client/generate/'.encode_url($cl->id_client))?>">Generate Code</a>
        </td>
      </tr>
    <?php $no++; } ?>
  </table>
